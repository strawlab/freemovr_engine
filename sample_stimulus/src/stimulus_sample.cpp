...
class StimulusSample: public StimulusInterface
{
public:
std::string name() const {
	return "StimulusSample";
}

virtual void post_init(std::string config_data_dir) {
	_virtual_world = create_virtual_world(config_data_dir);
	std::cout << "StimulusSample: _virtual_world created" << std::endl;
}
...
osg::ref_ptr<osg::Group> create_virtual_world(std::string config_data_dir)
{
  ...
  std::string basepath = config_data_dir+"/Sorsele3";
  add_skybox(top,basepath,".jpg");
...
POCO_EXPORT_CLASS(StimulusSample)
POCO_END_MANIFEST
