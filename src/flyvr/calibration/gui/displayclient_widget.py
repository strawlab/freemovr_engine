...
if platform.system() == "Windows":
    ...
    raise NotImplementedError(
        "Implement Windows ROSFreeDisplayServerProxy first!")
    ...
    sprite_pub = rospy.Publisher('sprite_image', flyvr.msg.FlyVRCompressedImage,
                                 latch=True)
...
class DisplayClientWidget(Gtk.VBox):
    ...
    __gsignals__ = {
        "displayclient-connect": (GObject.SignalFlags.RUN_FIRST, None, [object,
                                                                        bool]),
    ...
    def __init__(self,
                 interval_ms=1000,
                 default_displayserver="/display_server0"):
        ...
        ui_file_contents = pkgutil.get_data('flyvr.calibration.gui',
                                            'pinhole-wizard.ui')
        ui = Gtk.Builder()
        ...
        self._dsc_connect_btn = ui.get_object(
            'connect_to_display_server_button')
        self._dsc_disconnect_btn = ui.get_object(
            'disconnect_from_display_server_button')
        ...
        self._dsc_connect_btn.connect("clicked",
                                      self.on_connect_to_display_server)
        self._dsc_disconnect_btn.connect(
            "clicked", self.on_disconnect_from_display_server)
        ...
        self._fake_image.set_from_stock("gtk-missing-image",
                                        Gtk.IconSize.DIALOG)
    ...
    def update_dsc_sensitivity(self, is_connected):
        self._dsc_status_img.set_from_stock(Gtk.STOCK_YES if is_connected else
    ...
    def on_connect_to_display_server(self, *args):
        ...
        if ds_name is "" or ds_name is None:
            ...
            try:
                ...
                self.emit('displayclient-connect', self._display_server_proxy,
                          import_display_and_geometry_info)
    ...
    def proxy_set_display_server_proxy(self, display_server_name=None):
        ...
        if display_server_name is None:
            ...
            try:
                self._dsc = self._display_server_proxy = get_display_server_proxy(
                    display_server_name)
    ...
    def on_background_update(self, *args):
        ...
        self._draw_finish()
        print "[INFO] DisplayClientWidget: background update took %fsec" % (
            time.time() - start)
    ...
    def _draw_polygon_func(self, color, polygon_vertices=None):
        ...
        if polygon_vertices is None:
            ...
            self._draw_array = ((~mask[:,:, np.newaxis]) * self._draw_array
                                ) + (mask[:,:, np.newaxis] * color)
