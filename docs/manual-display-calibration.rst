...
::
    ...
    rosrun freemoovr_engine display_server
    rosrun freemoovr_engine joystick_cursor
...
with the example program in ``freemoovr_engine/homelab/picop-corners.py``.
...
The example program in ``freemoovr_engine/homelab/picop-corners.py`` run's
...
``freemoovr_engine/homelab/picop-corners.py`` example program, this will
...
model.::
    ...
    rosrun freemoovr_engine caldc4_manual_camera_calibration --image /tmp/display_server.json --geometry ../homelab/geom.json --camera picop

    # for a virtual display
    rosrun freemoovr_engine caldc4_manual_camera_calibration --image /tmp/display_server.json --geometry ../homelab/geom.json --camera picop/vdisp
...
Once you have obtained a good alignment, save the results.::
    ...
    # for the (entire) physical display
    CAMERA=picop rosrun freemoovr_engine record_camcal.sh
...
Now we have all the data we need. Create the texture coordinates::
    ...
    rosrun freemoovr_engine caldc6_create_display2tcs.py geom.json picop-camcal.bag
...
Now, show our new image.::
    ...
    rosrun freemoovr_engine show_image.py display2tcs-picop-tcs-lowres.png
